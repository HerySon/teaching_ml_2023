def encoding_vars(dataset, ohe = True, big_memory = False, cols_to_split_but_big_memory = ["categories", "ingredients_tags"],
                 ...
                 to_drop = ["creator", "abbreviated_product_name", "brands", "categories_tags", "origins_tags", "origins", "categories_fr",
    ...
    Features with ordinal order are encoded with OrdinalEncoder, the values present in theses features represent an hierarchical order which is important to keep.
    
    Features without ',' are encoded with OneHotEncoder, they have a correct number of distinct values and they will not cause curse of dimensionnality. 
    
    Features with ',' are manually encoded, like a OneHotEncoder would have done, but before we split the datas. 
    ...
    Args :
        ...
        cols_to_split -- list of columns containing values separated with ',' who represent a list of features. 
        Basically, we will split the values and each value will become a new feature.
        ...
        dummies_cols -- list of columns to process with OneHotEncoder. 
        (due to increasing dimensions resulting in OneHotEncoding, we only OneHotEncode features with a limited number of distinct values)  
        
        to_drop -- list of columns to drop from dataset. (redundant features or old features who have been now processed)
    ...
    # Splits, as explained in docstrings, we split by delimiter the values of the columns in different columns, 
    # we concat each columns into one to create a list of distinct values, then we loop into each values of the initials columns to fill the new ones.
